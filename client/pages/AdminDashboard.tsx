...
export default function AdminDashboard() {
  const { authState, adminSettings, updateAdminSettings, getAllUsers, createUser, updateUser, deleteUser, toggleUserStatus } = useAuth();
  ...
  const stats = {
    ...
    totalComments: state.posts.reduce((acc, post) => acc + post.comments.length, 0),
    totalReactions: state.posts.reduce((acc, post) => 
      acc + Object.values(post.reactions).reduce((sum, reaction) => sum + reaction.count, 0), 0
    )
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {stats.activeUsers} সক্রিয়, {stats.totalUsers - stats.activeUsers} নিষ্ক্রিয়
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">মোট রিয়াকশন</CardTitle>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.admins + stats.moderators + stats.editors}</div>
              <p className="text-xs text-muted-foreground">
                {stats.admins} এডমিন, {stats.moderators} মডারেটর, {stats.editors} এডিটর
              </p>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewUser(prev => ({ ...prev, phoneOrEmail: e.target.value }))}
                ...
                <div className="space-y-4">
                  {state.posts.map((post) => (
                    <div key={post.id} className="p-4 border rounded-lg">
                      ...
                      {post.image && (
                        <img src={post.image} alt="Post" className="rounded-lg mb-2 max-h-40 object-cover" />
                      )}
                      
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        <span>{Object.values(post.reactions).reduce((sum, r) => sum + r.count, 0)} রিয়া���শন</span>
                      ...
                      {post.comments.length > 0 && (
                        <div className="mt-4 space-y-2">
                          ...
                          {post.comments.map((comment) => (
                            <div key={comment.id} className="flex justify-between items-start p-2 bg-gray-50 rounded">
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateAdminSettings({ appName: e.target.value })}
                  />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">নতুন ইউজার রেজিস্টার করতে পারবে</p>
                    </div>
                    <Switch
                      checked={adminSettings.allowRegistration}
                      onCheckedChange={(checked) => updateAdminSettings({ allowRegistration: checked })}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">ইউজাররা পোস্ট করতে পারবে</p>
                    </div>
                    <Switch
                      checked={adminSettings.allowPosts}
                      onCheckedChange={(checked) => updateAdminSettings({ allowPosts: checked })}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">ইউজাররা স্টোরি পোস্ট করতে পারবে</p>
                    </div>
                    <Switch
                      checked={adminSettings.allowStories}
                      onCheckedChange={(checked) => updateAdminSettings({ allowStories: checked })}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label>কমেন্ট</Label>
                      <p className="text-sm text-gray-500">ইউজাররা কমেন্ট করতে পারবে</p>
                    </div>
                    <Switch
                      checked={adminSettings.allowComments}
                      onCheckedChange={(checked) => updateAdminSettings({ allowComments: checked })}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">ইউজাররা রিয়াকশন দিতে পারবে</p>
                    </div>
                    <Switch
                      checked={adminSettings.allowReactions}
                      onCheckedChange={(checked) => updateAdminSettings({ allowReactions: checked })}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label>মডারেশন</Label>
                      <p className="text-sm text-gray-500">পোস্ট এবং কমেন্ট অটো মডারেশন</p>
                    </div>
                    <Switch
                      checked={adminSettings.moderationEnabled}
                      onCheckedChange={(checked) => updateAdminSettings({ moderationEnabled: checked })}
                    />
                  </div>
                </div>
              </CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-red-600">{inactiveUsers.length}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
