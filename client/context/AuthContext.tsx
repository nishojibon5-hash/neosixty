...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [adminSettings, setAdminSettings] =
    React.useState(initialAdminSettings);
  ...
  const login = async (credentials: LoginCredentials): Promise<boolean> => {
    ...
    const user = users.find(
      (u) =>
        (u.phoneNumber === credentials.phoneOrEmail ||
          u.email === credentials.phoneOrEmail) &&
  ...
  const register = async (data: RegisterData): Promise<boolean> => {
    ...
    const newUser: User = {
      ...
      phoneNumber: data.phoneOrEmail.includes("@")
        ? undefined
        : data.phoneOrEmail,
  ...
  const createUser = async (
    userData: RegisterData & { role: UserRole },
  ): Promise<boolean> => {
    ...
    const existingUser = users.find(
      (u) =>
        u.phoneNumber === userData.phoneOrEmail ||
        u.email === userData.phoneOrEmail,
    );
    ...
    const newUser: User = {
      ...
      phoneNumber: userData.phoneOrEmail.includes("@")
        ? undefined
        : userData.phoneOrEmail,
      email: userData.phoneOrEmail.includes("@")
        ? userData.phoneOrEmail
        : undefined,
  ...
  const toggleUserStatus = async (userId: string): Promise<boolean> => {
    ...
    toast.success(
      `ইউজার ${users[userIndex].isActive ? "সক্রিয়" : "নিষ্ক্রিয়"} করা হয়েছে`,
    );
    return true;
  };
