...
const initialPosts: Post[] = [
  {
    ...
    content:
      "Just finished building my first React Native app! 🚀 The journey from web development to mobile has been incredible. Can't wait to share it with everyone soon.",
    isHtml: false,
    timeAgo: "2 hours ago",
    reactions: {
  ...
  },
  {
    ...
    content:
      '<div style="padding: 20px; background: linear-gradient(45deg, #ff6b6b, #4ecdc4); border-radius: 10px; color: white; text-align: center;"><h2>🌅 Beautiful Sunset</h2><p>Sometimes we need to pause and appreciate the simple moments in life.</p><p><strong>What made you smile today?</strong></p></div>',
    isHtml: true,
    image: "/placeholder.svg",
    timeAgo: "4 hours ago",
    reactions: {
...
type Action =
  ...
  | {
      type: "ADD_REACTION";
      payload: { postId: string; reactionType: ReactionType };
    }
  | {
  ...
  | {
      ...
      payload: { userId: string; increment: boolean };
    }
  | {
      type: "UPDATE_VERIFICATION";
      payload: { userId: string; isVerified: boolean };
    }
        ...
        posts: state.posts.map((post) => {
          if (post.id === action.payload.postId) {
            ...
            reactions[reactionType] = {
              ...
              users: reactions[reactionType].users.filter(
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const addComment = (
    ...
    dispatch({
      type: "ADD_COMMENT",
      payload: { postId, content, image, video },
    });
  };
  ...
  const checkAndUpdateVerification = (userId: string) => {
    ...
    if (user && user.followerCount >= 1000 && !user.isVerified) {
      dispatch({
        type: "UPDATE_VERIFICATION",
        payload: { userId, isVerified: true },
      });
      if (userId === state.currentUser.id) {
        toast.success(
          "Congratulations! You've been verified with a blue badge!",
        );
      }
    }
  };
